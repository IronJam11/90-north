...
from api.chats.models import Room
...
def create_room(request):
    try:
        ...
        enrollment_no_or_error = get_userdetails_from_token(token)
...
def add_participants_to_room(request):
    try:
        ...
        workspace = WorkSpace.objects.filter(roomname=room_slug).first()
        
        if workspace:
            # Add users to the project's participants
            workspace.participants.add(*users_to_add)
            workspace.save()
